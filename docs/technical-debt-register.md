...
| REL-001 | P1 | CI Android JVM | `SystemBarInsetPolicyTest` y `WorkoutQuickActionContractTest` existian, pero el required quality gate solo ensamblaba APKs. El gate ejecuta ahora `:app:testDebugUnitTest` completo antes del assemble y conserva su XML JUnit por 30/90 dias. | Mitigado; retirar la tarea o acoplarla silenciosamente al assemble reabriria el gap. | Mantener tarea JVM explicita, fallo autoritativo y artifact durable; el contrato del quality gate protege esta secuencia. | CLOSED / GUARDED |
| REL-002 | P1 | Evidencia E2E | El primer intento del run `33668698934` produjo evidencia Playwright local que no quedo durable; el fallo no se reprodujo y su causa sigue `UNKNOWN`. El gate sube ahora `test-results/` cuando falla Axe, E2E o Web-dist E2E y existe una politica explicita de flakes. | Mitigado; la evidencia depende de lo que Playwright alcance a generar antes del fallo. | Conservar traces/contexto sin retries silenciosos; tratar reruns diagnosticos como observaciones registradas, no como sustitutos del fallo original. | CLOSED / GUARDED |
