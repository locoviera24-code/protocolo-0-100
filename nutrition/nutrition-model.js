(function(global){
  ...
  function buildEntry(food,grams,meal,date,{id,now,definitions}={}){
    ...
    return{id:id||`food_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,foodId:food.id||'',fdcId:food.fdcId||null,recipeId:food.recipeId||null,recipeSnapshot:food.recipeSnapshot||null,date,meal:MEALS.includes(meal)?meal:(meal==='Otro'?'Otros':'Otros'),name:String(food.name||'').trim(),grams:amount,calories:Math.round(number(food.calories)*factor+1e-9),protein:round(number(food.protein)*factor),carbs:round(number(food.carbs)*factor),fat:round(number(food.fat)*factor),nutrients,nutrientStatus,source:food.source||'registro manual',sourceCitation:food.sourceCitation||'',savedAt:now||new Date().toISOString()};
  }
