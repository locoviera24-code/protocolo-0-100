...
const qualityChannel=String(process.env.QUALITY_CHANNEL||'development').toLowerCase();
if(qualityChannel==='stable')assert.equal(stableRelease.build,appVersion.build,'La solicitud estable debe usar el build activo');
else assert.ok(stableRelease.build<=appVersion.build,'Una beta no puede quedar por debajo del build estable solicitado');
