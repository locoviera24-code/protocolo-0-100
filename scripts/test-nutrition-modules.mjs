...
const window={APP_REPOSITORIES:{nutrition:nutritionRepository},NUTRIENT_DEFINITIONS:{fiber:{label:'Fibra'},iron:{label:'Hierro'}}};
...
const food={id:'mandioca',name:'Mandioca hervida',aliases:['mandiócas'],calories:125,protein:1,carbs:30,fat:.3,fiber:1.8,confidence:'alto',units:{taza:200}};
...
assert.equal(entry.calories,250);assert.equal(entry.nutrients.fiber,3.6);assert.equal(entry.nutrientStatus.iron,'unknown');
...
const coverage=confidence.coverage([entry],['fiber','iron']);
assert.equal(coverage.rows.find(row=>row.key==='iron').unknown,1);assert.notEqual(coverage.confidence,'high');
