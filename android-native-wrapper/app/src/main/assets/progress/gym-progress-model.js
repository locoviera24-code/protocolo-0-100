(function(global){
  ...
  function flatten(sessions,library=[]){
    ...
    (sessions||[]).forEach(session=>(session.exercises||[]).forEach(exercise=>{
      ...
      const sets=(exercise.sets||[]).map((set,index)=>{const reps=Math.max(0,data().number(set.reps)),weight=Math.max(0,data().number(set.weightKg??set.weight));return{id:set.id||`${session.id}-${exercise.id}-${index}`,reps,weight,volume:reps*weight,isBodyweight:!!(set.isBodyweight||set.bodyweight||exercise.bodyweight||definition?.unit==='peso corporal')};});
