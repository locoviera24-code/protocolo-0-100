(function(global){
  'use strict';
  const data=()=>global.PROGRESS_DATA_MODEL,taxonomy=()=>global.MUSCLE_TAXONOMY,setModel=()=>global.WORKOUT_SET_MODEL;
  ...
  function flatten(sessions,library=[]){
    ...
    (sessions||[]).forEach(session=>(session.exercises||[]).forEach(exercise=>{
      ...
      const sets=(exercise.sets||[]).map((raw,index)=>{const set=setModel()?.normalize?.(raw)||{...raw,setType:raw.setType||'working',completed:raw.completed!==false},reps=Math.max(0,data().number(set.reps)),weight=Math.max(0,data().number(set.weightKg??set.weight));return{id:set.id||`${session.id}-${exercise.id}-${index}`,reps,weight,volume:reps*weight,rir:set.rir??null,rpe:set.rpe??null,note:String(set.note||set.notes||''),setType:set.setType,completed:set.completed,mainVolume:setModel()?.countsMainVolume?.(set)??set.completed,recordEligible:setModel()?.countsForRecords?.(set)??set.completed,progressionEligible:setModel()?.countsForProgression?.(set)??set.completed,excludeFromRecords:!!set.excludeFromRecords,excludeFromProgression:!!set.excludeFromProgression,isBodyweight:!!(set.isBodyweight||set.bodyweight||exercise.bodyweight||definition?.unit==='peso corporal')};});
