(function(){
  ...
  function renderPersonalRecords(){const records=window.PERSONAL_RECORDS.build(exerciseModel()),box=document.getElementById('progressRecordsList'),unit=window.WORKOUT_FEATURES?.getGymSettings?.().unit||'kg',value=record=>record.measure==='reps'?`${record.value} reps`:record.measure==='duration'?formatProgressDuration(record.value):record.measure==='distance'?(record.value>=1000?`${Math.round(record.value/10)/100} km`:`${Math.round(record.value)} m`):record.measure==='volume'?`${window.WORKOUT_FEATURES?.displayVolume?.(record.value,unit)??record.value} ${unit}`:`${window.WORKOUT_FEATURES?.displayWeight?.(record.value,unit)??record.value} ${unit}`;box.innerHTML=records.length?records.map(record=>`<article class="progressRecordRow"><div><strong>${escape(record.name)}</strong><span>${escape(record.label)}</span></div><b>${escape(value(record))}</b></article>`).join(''):'<div class="emptyState">Registrá series para derivar récords personales.</div>';}
