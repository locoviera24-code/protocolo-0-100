(function(){
  ...
  function setup(){
    ...
    window.addEventListener('app-data-change',event=>{
      const affected={protocol:['overview','habits','history','achievements'],workout:['overview','gym','history'],nutrition:['overview','nutrition','history'],import:[...views]}[event.detail?.domain];
      if(!affected)return;markDirty(affected);if(progressRoute()&&affected.includes(activeView))requestAnimationFrame(render);
    });
    window.addEventListener('app-route-change',event=>{if(event.detail?.module==='progress')applyRoute(event.detail.view,{focus:true});});
